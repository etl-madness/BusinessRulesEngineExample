...
public class SqlDatabaseService : IBusinessRuleStore
{
    ...
    public async Task<IEnumerable<DbConnectionDefinition>> GetAllDbConnectionsAsync()
    {
        ...
        {
            if (_encryptionService is AesEncryptionService)
            {
                try
                {
                ...
                {
                    Console.WriteLine($"Error decrypting connection string for {conn.Name}: {ex.Message} assuming encryption is not applied.");
                }
            }
        }
        return conns;
