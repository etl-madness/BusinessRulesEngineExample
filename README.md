...
A Blazor Server application demonstrating the usage of `NuGet` `EtlAnalytics.RulesEngine.2.1.0` for managing and executing TSQL and C# business rules, specifically refactored for KVE (Known Vulnerability Exploitation) reporting.
...
- Encryption is not required for this demo, but you can enable it via setting an `DB_ENCRYPTION_KEY` variable in your `Properties/launchSettings.json`, or Define via Environment if needed. It can also be set in `appsettings.json` if prefered.
```json
 {
    "Security": {
        "EncryptionKey": "A_Secure_Key_For_Connection_Strings_123!"
    }
}
...
- If you want to seed the database with example KVE/CVE data, run `3. KVE Reporting Data Schema` and `4. Example Data (Seed)`. This is not needed for the Rules Engine to function, but it provides sample data for testing.
